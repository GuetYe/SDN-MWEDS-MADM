...
LastEditTime: 2024-04-06 11:05:17
LastEditors: 孙石泉
Description: 1.控制器获取samba服务器的状态信息  2.获取终端上传文件的请求并根据多属性决策得到分割方案，把分割方案发给终端
FilePath: \SD-UANET_load_balance\controller\ryu_operation\host_get_msg.py
'''
...
class Host_Get_MSG(app_manager.RyuApp):
    ...
    def packet_in_handler(self, ev):
        ...
        if self.search_server_stats_method.search(data_str) != None:  
            # print("<host_get_msg.py>   获取服务器的状态信息")
            ...
            print("<host_get_msg.py>  the samba server {}'s status info：{}".format(src_host_ip, self.all_host_stats[src_host_ip]))

        # 2.检测是否是OVS交换机的状态信息
        elif self.search_switch_stats_method.search(data_str) != None:
            # print("<host_get_msg.py>   获取OVS交换机的状态信息")
            ...
            print("<host_get_msg.py>  the OVS switch {}'s statue info：{}".format(host_num, self.all_switch_stats[host_num]))
        ...
        # 3.检测是否是Client的存储文件请求
            ...
            if dst_cont_ip in setting.CONTROLLER_IP:
                ...
                print("<host_get_msg.py>  request to transfer files!")
                ...
                if datapath == None:
                    print("<host_get_msg.py> the datapath doest't exist")
                    return
                print("\n★ ★ ★ ★ ★")
                print("<host_get_msg.py>  receive a transfer request from the client!")
                ...
                if split_result:
                    ...
                    print("<host_get_msg.py>   the split result is returned to the client %s" % src_host_ip)
                    print("★ ★ ★ ★ ★ ★ \n")
