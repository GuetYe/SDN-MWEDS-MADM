...
LastEditTime: 2024-04-17 10:41:13
LastEditors: 孙石泉
Description: 
FilePath: \SD-UANET_load_balance\controller\ryu_operation\network_structure.py
'''
...
class Networkstructure(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        self.logger.info("<network_structure.py> ---> switch {} connected and install table miss entry".format(datapath.id))
    ...
    def _state_change_handler(self, ev):
        ...
        if ev.state == MAIN_DISPATCHER:
            if datapath.id not in self.sw_datapaths_table:
                # 保存交换机的datapath实例
                self.sw_datapaths_table[datapath.id] = datapath
                self.logger.info("<network_structure.py> --->  register datapath: %016x" % datapath.id)
        ...
        # 联机中断
            ...
            #     del self.sw_datapaths_table[datapath.id]
                # self.logger.info("<network_structure.py> --->  unreigster datapath: %016x" % datapath.id)
                # self.logger.info("<network_structure.py> --->  unreigster datapath: %016x" % datapath.id)
        # 交换机状态正常，允许所有操作交换机的行为
        self.sw_change_flag = False
    ...
    def get_topology(self, ev, way=0):
        ...
        if not self.sw_dpid_list:
            self.logger.info("<network_structure.py> --->  There are not have switch, please wait it")
            return
        ...
        if switch_list == [] or link_list == {}:
            print("<network_structure.py> --->   switch_list or link_list is empty")
            return
        ...
        for each_switch in switch_list:
            ...
            # print("<network_structure.py> --->   each_switch: ", each_switch)
        ...
        #     print("<network_structure.py> --->  link_table reset")
        #     self.link_table = {}
        #     self.get_topology_num = 0
        ...
        for link in link_list:
            # print("<network_structure.py> --->   link: ", link)
        ...
        if len(self.link_table) != 0:
            print("<network_structure.py> ---> the link_table to assignment is:", self.link_table)
            self.link_table_backup = self.link_table
        else:
            print("<network_structure.py> ---> link_table(zero) is:", self.link_table)
            print("<network_structure.py> ---> link_table_backup is:", self.link_table_backup)
        ...
        #             self.logger.info("the number of agree to delete dpid:{}".format(self.delete_sw_info[each_dpid]['delete_counter']))
        #             # 触发删除条件的次数>设定的阈值
        ...
        #                 self.logger.info("the number of agree to delete dpid:{}".format(self.delete_sw_info[each_dpid]['delete_counter']))
        #                 self.logger.info("<structure.py> ---> topology was changed,remode {}'s information".format(each_dpid))
        ...
        if way == 0:
            self.logger.info("<structure.py> ---> network topology build passively")
        else:
            self.logger.info("<structure.py> ---> network topology build actively")

        logger = self.logger.info if setting.LOGGER else print
        setting.print_pretty_table(self.sw_ports_table , ['dpid', 'port_no'], [10, 10],
                    '<structure.py> --- sw_ports_table', logger)  # 交换机所有端口
        setting.print_pretty_table(self.link_table, ['dpid-dpid', 'port-port'], [10, 10],
                                   '<structure.py>--- link_table', logger)  # 交换机相互连接的端口
        setting.print_pretty_table(self.sw_link_port_table, ['dpid', 'port'], [10, 10],
                                   '<structure.py>--- sw_link_port_table', logger)  # 交换机相互连接的端口
        setting.print_pretty_table( self.delay.link_delay, ['sw---sw', 'delay'], [10, 10],
                            '<structure.py>--- link_delay', logger)  # 交换机之间的时延
        setting.print_pretty_table(self.access_table, ['dpid-port_no   ', '   ip-mac'], [10, 10],
                            '<structure.py>--- self.access_table', logger)  # 主机与交换机的连接信息
        setting.print_pretty_table(self.sw_datapaths_table, ['dpid', 'datapath'], [10, 10],
                                   
                                   '<structure.py> --- sw_datapaths_table', logger)  # 交换机所有datapaths
        
        print("<structure.py>---not_use_ports:{}".format(self.not_use_ports)) # 交换机没有使用的端口

        print("<structure.py>---sw_dpid_list:{}".format(self.sw_dpid_list))
        self.logger.info("<structure.py> The network topology is:{}".format(self.network_topology))
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth_type == ether_types.ETH_TYPE_LLDP:
            # self.logger.info("<shortest_path.py>--->packet-in:LLDP Packet is detected, return") # 忽略LLDP报文
            return
        ...
        if isinstance(arp_pkt, arp.arp): 
            ...
            print("<structure.py>--->arp_src_ip_src_ip:{}".format(arp_src_ip))
