...
LastEditTime: 2024-03-07 16:07:20
LastEditors: Please set LastEditors
Description: 
FilePath: /SD-UANET_load_balance/SDN/ryu_operation/network_delay.py
'''
import time
...
class NetworkDelayDetector(app_manager.RyuApp):
    ...
    def _ehco_reply_handler(self, ev):
        ...
        try:
            ...
            self.logger.warning("<monitor.py>--->calculate echo_delay error:%s", error)        
        ...
        try:
            ...
            for port in self.switch_module.ports.keys():
                if src_dpid == port.dpid and src_port_no == port.port_no:
                    ...
                    # self.logger.warning("<monitor.py>--->calculate lldp_delay successfully") 
                    self.calculate_delay(src_dpid, dpid)
                        
        except LLDPPacket.LLDPUnknownFormat as e:
            self.logger.warning("<monitor.py>--->calculate lldp_delay error") 
            return
    ...
    def calculate_delay(self, src, dst):
        ...
        sw_to_sw_delay = (lldp_delay_forward + lldp_delay_reverse - ech0_delay_src - ech0_delay_dst) / 2
        # 计算的时延为负数，则默认为0.001s
        if sw_to_sw_delay < 0:
            sw_to_sw_delay = 0.001
