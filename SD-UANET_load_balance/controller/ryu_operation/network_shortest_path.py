...
LastEditTime: 2024-05-25 11:37:26
LastEditors: 孙石泉
Description: 
...
class ShortestPathForwarding(app_manager.RyuApp):
    ...
    def send_flow_mod_first(self, datapath, eth_type, eth_src, src_ip, dst_ip, src_port, dst_port, second_node_num):
        ...
        # 
        actions = [parser.OFPActionPushMpls(ethertype=eth_type), parser.OFPActionSetField(mpls_label=setting.MPLS_LABEL),\
    ...
    def send_flow_mod_two_to_before_last(self, datapath, eth_type, eth_src, src_port, dst_port, node_num, next_node_num):
        ...
        # , eth_dst=eth_dst_match
        
        match = parser.OFPMatch(in_port=src_port, eth_type=eth_type, mpls_label=setting.MPLS_LABEL,
    ...
    def send_flow_mod_last(self, datapath, eth_type, eth_src, dst_ip, src_port, dst_port, node_num):
        ...
        actions = [parser.OFPActionPopMpls(ethertype=ether_types.ETH_TYPE_IP), \
                   parser.OFPActionSetField(eth_dst=eth_dst), \
                   parser.OFPActionOutput(dst_port)]
    ...
    def get_port_pair(self, src_dpid, dst_dpid):
        ...
        if (src_dpid, dst_dpid) in self.structure.link_table_backup.keys():
            ...
            print("<shortest_path.py>--->  link_table_backup is:", self.structure.link_table_backup)
            print("<shortest_path.py>--->get_port_pair: dpid: %s -> dpid: %s is not in links"% (src_dpid, dst_dpid))
            return None
    ...
    def get_switches(self, dpid, in_port, src_ip, dst_ip):
        ...
        '''
        dst_switch = list()
        print("<shortest_path.py>--->get_switches:get src-{} and dst-{} switch".format(src_ip, dst_ip))
        ...
        if src_location != None and src_location[0] == dpid:
            ...
            print("<shortest_path.py>--->get_switches:get src_location{} error".format(src_ip))
            return None,None

        # # 触发packet-in的交换机端口可能不是连接主机的端口
        if in_port not in self.structure.not_use_ports[dpid]: 
            print("<shortest_path.py>->get_switches：not_use_ports[{}]：{}".format(dpid, 
                            self.structure.not_use_ports[dpid]))
            print("<shortest_path.py>->get_switches：in_port-{} is not in not_use_ports[{}]".format(in_port, 
                            dpid))
            return None, None
        ...
        if dst_ip in setting.DST_MULTICAST_IP.keys():
            ...
            for other_ip in dst_group_ip:
                ...
                if dst_location:
                    ...
                    print(f"<shortest_path.py>--->get multicast route dst switches error: {dst_ip}")
                    return None, None
            print(f"<shortest_path.py>--->get multicast route dst switches: {dst_switch}")
        # 单播的目的节点
        else:
            ...
            if dst_location:
                ...
                print(f"<shortest_path.py>--->get single route dst switches error: {dst_ip}")
                return None, None
        # 获取源节点和目的节点的交换机成功，打印相关信息查看是否有误
        print("\n")
        print("<shortest_path.py>--->get_switches->src_ip:{},src_switch:{}".format(src_ip, src_switch))
        print("<shortest_path.py>--->get_switches->dst_ip:{},dst_switch:{}".format(dst_ip, dst_switch))
        print("\n")
    ...
    def install_flow(self, path, eth_type, eth_src, src_ip, dst_ip, in_port, buffer_id, data=None):
        '''
        ...
        if path is None or len(path) == 0:
            print("<shortest_path.py>--->install_flow error: Path Error")
            return False
        else:
            ...
            if len(path) > 2:
                for i in range(1, len(path) - 1):
                    ...
                    if port_pair and port_pair_next:
                        ...
                        print(f"<shortest_path.py>--->install_flow error: len(path) > 2 "
            ...
            if len(path) > 1:
                ...
                print("<shortest_path.py>---> path:{}, port_pair:{}, in_port:{}".format(path, port_pair, in_port))

                if port_pair is None:
                    print("<shortest_path.py>--->install_flow error in len(path)>1: port not found")
                    return False
                ...
                if dst_port is None:
                    print("<shortest_path.py>--->install_flow error in len(path)>1: last port is not found")
                    return False
                # 给最后1个交换机下发流表
                ...
                if port_pair is None:
                    print("<shortest_path.py>--->install_flow error in len(path)>1: port not found in first switch")
                    return False
                # 第1个交换机的输出端口
            ...
            # 3.len(path) == 1，给第1个交换机下发流表项
                ...
                if out_port is None:
                    print("<shortest_path.py>--->install_flow error: out_port is None in first switch")
                    return False
                self.send_flow_mod(first_dp, eth_type, src_ip, dst_ip, in_port, out_port)
                self.send_flow_mod(first_dp, eth_type, dst_ip, src_ip, out_port, in_port)
    ...
    def claculate_path_and_install_flow_entry(self, msg, eth_type, eth_src, src_ip, dst_ip):
        '''
        ...
        print("<shortest_path.py>---> src_ip:{} dst_ip:{}".format(src_ip, dst_ip))
        print("<shortest_path.py>---> the switch that triggers packet-in is：{}".format(datapath.id))
        # 如果源节点和目的节点存在，计算最优路径，下发流表
        if src_dst_switches[0] != None and len(src_dst_switches[1]) >= 1:
            src_switch, dst_switch = src_dst_switches
            print("<shortest_path.py>--->{} ping {}, src_dst_switches:{}".format(src_ip, dst_ip, src_dst_switches))
            # 1.单播情况
            if len(dst_switch) == 1:
                dst_switch = dst_switch[0]
                # 计算最优路径，综合考虑剩余带宽、时延、丢包率
                path = self.get_shortest_paths(src_switch, dst_switch, weight="synthetic_weight")
                print("<shortest_path.py>--->[path]%s--%s: %s"% (src_switch, dst_switch, path))
                # 下发流表
                install_flow_result = self.install_flow(path, eth_type, eth_src, src_ip, dst_ip, in_port, msg.buffer_id, msg.data)
                if install_flow_result == True:
                    print("<shortest_path.py>--->The installation of the path's flow entry was successful!")
                    return True
                else:
                    return False
            ...
            # 2.组播情况，dst_switch为列表
                ...
                print("<network_shortest_path.py>->multicast path is not available at present")
                return False
        ...
        #     print("<network_shortest_path.py>  The installation of the mesh's transfer flow entry was successful!")
        #     return False

        else:
            print("<network_shortest_path.py> ---> src_dst_switches is nonexistent")
            return False
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth_type == ether_types.ETH_TYPE_LLDP:
            # print("<network_shortest_path.py> --->  dpid:{} eth_pkt:{}".format(datapath.id, eth_pkt))
            return

        print("<network_shortest_path.py> ---> \n") 
        # print("<network_shortest_path.py> ---> eth_type:", eth_type) 
        ...
        if isinstance(ipv4_pkt, ipv4.ipv4) or eth_type == ether_types.ETH_TYPE_IP:
            ...
            # print("<network_shortest_path.py> ---> src_ip and dst_ip is:", src_ip, dst_ip)
            # print("<network_shortest_path.py> ---> packet_cache is:", self.packet_cache)
            ...
            # print("<network_shortest_path.py> ---> src_ip:{} dst_ip:{}".format(src_ip, dst_ip))
            ...
            if (src_ip, dst_ip) in self.packet_cache.keys():
                if time.time() - self.packet_cache[(src_ip, dst_ip)] <= setting.INTERVAL_INSTALL_FLOW: 
                    # print("<network_shortest_path.py> ---> The interval for install flow entry is less than %d s, prohibit installing flow entry"% setting.INTERVAL_INSTALL_FLOW)
                    return False
            ...
            print("<network_shortest_path.py> ---> \n") 
            # 计算最短路径并下发流表
            install_flow_result = self.claculate_path_and_install_flow_entry(msg, eth_type, eth_src, src_ip, dst_ip)
