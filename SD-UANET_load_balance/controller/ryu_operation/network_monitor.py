...
LastEditTime: 2024-04-20 15:44:16
LastEditors: 孙石泉
Description: 
...
class Networkmonitor(app_manager.RyuApp):
    ...
    def port_desc_stats_reply_handler(self, ev):
        ...
        self.logger.info("<network_monitor> ---> -----port's config and stats-----")
        self.logger.info("dpid    port    config   state     curr_speed     max_speed")
        for ofport in body:  
            if ofport.port_no != ofproto_v1_3.OFPP_LOCAL:  # 0xfffffffe  4294967294,本地回环端口
                ...
                try:
                    ...
                    self.logger.info("<network_monitor>  ofport.curr_speed:%d  ofport.max_speed:%d", ofport.curr_speed, ofport.max_speed)
                # 端口流速有时在properties中
                except:  
                    ...
                    self.logger.info("<network_monitor>  properties[0].curr_speed:%d  properties[0].max_speed:%d", ofport.properties[0].curr_speed, ofport.properties[0].max_speed)
                # 无线端口时，无线网卡不给出最大带宽
                ...
                self.logger.info(" %d        %d      %s      %s      %.3f    %.3f", dpid, ofport.port_no, config, state, 
    ...
    def port_stats_table_reply_handler(self, ev):
        ...
        self.logger.info("<network_monitor.py> -----switch port's remain bandwidth -----")
        self.logger.info("switch  port_no  rx_bytes tx_bytes  used_bw  used_bw_radio  remain_bw tx_packets rx_packets")
    ...
    def calculate_remain_bw(self, dpid, port_no, stats):
        ...
        if port_state:
            ...
            self.logger.info("  %d      %d  %d %d %d    %.2f     %d  %d  %d %d", 
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        msg = ev.msg
        body = msg.body
        self.logger.info("<network_monitor.py> ----- switch port's flow entry -----")
        self.logger.info("dpid  table_id   priority   eth_type   in_port   mpls_label    src_ip         dst_ip         out_port    count")
        # 获取交换机每个流表的信息
        for each_state in body:

            # print("<network_monitor.py> --- each_state:", each_state)
            ...
            if setting.SHOW_FLOW_ENTRY:
                self.logger.info(" %2d      %2d       %5d      %s   %s    %s   %s     %s     %s    %s", 
                                 dpid, table_id, priority, eth_type, in_port, mpls_label, src_ip, dst_ip, out_port, count)
    ...
    def _port_status_handler(self, ev):
        ...
        if msg.reason == ofp.OFPPR_ADD:
            ...
            reason = 'unknown'
        # %d  msg.desc,
        self.logger.info('<network_monitor.py >---> switch %016x port status has changed,reason:%s'% dp.id,  reason)
    ...
    def calculate_loss(self):
        ...
        for link, port in self.structure.link_table_backup.items():
            ...
            if (src_dpid, src_port) in self.port_stats_table.keys() and (dst_dpid, dst_port) in self.port_stats_table.keys():
                ...
                self.logger.info("<network_monitpr.py> ---> calculate loss error, {} is not in structure.link_table".format(link))
        
        self.logger.info("network_monitor.py---link_loss:{}".format(self.link_loss))
        self.logger.info("\n")
